...
def warn_updates():
    ...
    with open(config.PYDEFLATE_PATHS.data / r"data_updates.json") as file:
...
def update_update_date(source: str):
    ...
    if not (config.PYDEFLATE_PATHS.data / r"data_updates.json").exists():
        updates = {}
        with open(config.PYDEFLATE_PATHS.data / r"data_updates.json", "w") as outfile:
