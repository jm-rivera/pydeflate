...
def _generate_get_deflator_docstring(source_name: str, price_kind: str) -> str:
    ...
    return (
        f"Get deflator data from {source_name} "
        f"({price_kind}) without requiring "
        "user data.\n\n"
        "This function returns a DataFrame containing "
        "deflator values for the specified "
        "parameters.\n\n"
        "Args:\n"
        ...
        "currency code. Defaults to 'USA'.\n"
        "    countries (list[str] | None, optional): "
        ...
        "Defaults to False.\n"
        "    to_current (bool, optional): Get deflators "
        "for constant-to-current conversion. "
        "Defaults to False.\n"
        "    include_components (bool, optional): Include "
        "price_deflator, exchange_deflator, and "
        "exchange_rate columns. Defaults to False.\n"
        "    update_deflators (bool, optional): Update "
        "the deflator data before retrieval. "
        "Defaults to False.\n\n"
        "Returns:\n"
        ...
        "        - year: Year\n"
        "        - deflator: The combined "
        "deflator value\n"
        "        - price_deflator "
        "(if include_components=True): "
        "The price deflator component\n"
        "        - exchange_deflator "
        "(if include_components=True): "
        "The exchange rate deflator component\n"
        "        - exchange_rate "
        "(if include_components=True): "
        "The exchange rate\n"
    )
...
def _get_deflator(deflator_source_cls, price_kind):
    """Create get_deflator wrappers with specific source and price kind."""
