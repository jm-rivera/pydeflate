...
def _generate_docstring(source_name: str, price_kind: str) -> str:
    ...
    return (
        f"Deflate a DataFrame using the {source_name} "
        f"deflator source ({price_kind}).\n\n"
        f"This function applies deflation adjustments "
        f"to a DataFrame using the {source_name} "
        f"{price_kind} deflator.\n\n"
        "Args:\n"
        "    data (pd.DataFrame): The input DataFrame "
        "containing data to deflate.\n"
        "    base_year (int): The base year for "
        "calculating deflation adjustments.\n"
        "    source_currency (str, optional): The source "
        "currency code. Defaults to 'USA'.\n"
        "    target_currency (str, optional): The target "
        "currency code. Defaults to 'USA'.\n"
        "    id_column (str, optional): Column with "
        "entity identifiers. Defaults to 'iso_code'.\n"
        "    year_column (str, optional): Column with "
        "year information. Defaults to 'year'.\n"
        ...
        "Defaults to 'value'.\n"
        "    to_current (bool, optional): Adjust values "
        "to current-year values if True. "
        "Defaults to False.\n"
        "    year_format (str | None, optional): Format "
        "of the year in `year_column`. "
        "Defaults to None.\n"
        "    update_deflators (bool, optional): Update "
        "the deflator data before deflation. "
        "Defaults to False.\n\n"
        "Returns:\n"
        "    pd.DataFrame: DataFrame with deflated "
        "values in the `target_value_column`.\n"
    )


def _deflator(deflator_source_cls, price_kind):
    """Create deflate wrappers with specific source and price kind."""
