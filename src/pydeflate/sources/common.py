...
from pydeflate.sources._country_mapping import COUNTRY_NAME_TO_ISO3
...
def _match_name_to_iso3(
    to_match: list[str], additional_mapping: dict | None
) -> dict[str, str]:
    """Match a list of entity name strings to ISO3 country codes.

    Uses the static mapping generated by scripts/generate_country_mapping.py.

    Args:
        to_match (list): A list of entity name strings to match.
        additional_mapping (dict | None): Extra overrides merged on top.

    Returns:
        dict: A dictionary with the entity names as keys and the ISO3 codes as values.
    """
    mapping = dict(COUNTRY_NAME_TO_ISO3)
    if additional_mapping:
        mapping.update(additional_mapping)

    result: dict[str, str | None] = {}
    for name in to_match:
        iso3 = mapping.get(name)
        result[name] = iso3
        if iso3 is None:
...
def convert_id(
    ...
    *,
    ...
    mapping_functions = {"regex": _match_name_to_iso3}
