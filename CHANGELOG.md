...
### Changed
- **Replaced `hdx-python-country` runtime dependency with a static mapping**: Country name→ISO3 resolution now uses a pre-generated dictionary (`src/pydeflate/sources/_country_mapping.py`) instead of fuzzy matching at runtime. This eliminates a heavy transitive dependency tree (`hdx-python-utilities`, `frictionless`, `ijson`, `loguru`, etc.) and improves startup time and determinism. `hdx-python-country` is now a dev-only dependency used by `scripts/generate_country_mapping.py` to regenerate the mapping when needed. Historical name aliases (e.g., "Swaziland", "Burma", "Turkey") are included for compatibility with older cached data.
- **Distinguished Euro Area (EMU) from European Union (EU)**: Entity names like "Euro area" and "Euro Area (EA)" now map to `pydeflate_iso3="EMU"` instead of `"EUR"`. "European Union" variants map to `"EU"`. The user-facing currency code `"EUR"` (used in `source_currency`/`target_currency` parameters) is unchanged — it still resolves to the Euro Area internally. **Breaking**: code that filters source data by `pydeflate_iso3 == "EUR"` to find Euro Area rows must now use `"EMU"`. Users who only pass `source_currency="EUR"` / `target_currency="EUR"` are unaffected.
